...
export default function PlannerPage() {
  ...
  return (
    <div className="h-[calc(100vh-3rem)] grid grid-cols-[320px_1fr_380px] gap-3 p-3">
      <aside className="bg-white rounded-xl border overflow-hidden flex flex-col">
        <DaySidebar
          selectedDate={selectedDate}
          onChangeDate={setSelectedDate}
        />
      </aside>
      <main className="bg-white rounded-xl border overflow-hidden flex flex-col">
        <div className="flex items-center justify-between p-3 border-b">
          <div className="text-sm text-gray-500">中央: スケジュール</div>
          <Pomodoro />
        </div>
        ...
        <div className="border-t p-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <textarea
              className="min-h-[160px] border rounded p-2 text-sm"
              placeholder="Markdownでメモ... 自動保存"
              value={note}
              onChange={(e) => setNote(e.target.value)}
            />
            <div className="prose prose-sm max-w-none border rounded p-2">
